...
public static class ServiceCollectionExtensions
{
    ...
    /// <param name="services">The service collection to which the services will be added.</param>
    /// <param name="configure">An action to further configure resource access rules, caller context accessors, and resource object accessors using the <see cref="DependencyConfigurator{TResource}"/>.</param>
    /// <returns>The updated service collection.</returns>
    public static IServiceCollection ConfigureWincheSentinel<TResource>(this IServiceCollection services, Action<DependencyConfigurator<TResource>> configure) where TResource : class 
    {
        configure.Invoke(new DependencyConfigurator<TResource>(services));
